...
STRESSED_CONFIRM_COUNT = 1
STATE_MIN_HOLD_SECONDS = 0.45

# Fatigue is independent from emotion classification.
# IMPORTANT: eye detection uses the ORIGINAL YuNet face ROI, not the
# 112x112 aligned FER image. The aligned image was causing false CLOSED states.
STARTUP_EYE_WARMUP = 3.0
FATIGUE_SECONDS = 2.5
FATIGUE_RECOVERY_SECONDS = 0.8
EYE_HISTORY_SIZE = 10
FATIGUE_ENTER_OPEN_RATIO = 0.20
FATIGUE_EXIT_OPEN_RATIO = 0.60
NO_FACE_OFF_SECONDS = 1.0
...
def required_confirm_count(state):
    if state == "STRESSED":
        return STRESSED_CONFIRM_COUNT
    return STATE_CONFIRM_COUNT
...
def eyes_are_open(
    ...
    """
    Detect eyes from the ORIGINAL YuNet face ROI.

    The previous version searched the 112x112 FER-aligned image. That crop is
    excellent for expression recognition but unreliable for Haar eye detection,
    which caused OPEN eyes to be reported as CLOSED.
    """
    ...
    # Add a small margin around the YuNet box.
    mx = int(w * 0.08)
    my = int(h * 0.06)
    ...
    # Search only the upper 58% of the original face.
    upper_h = max(1, int(gray.shape[0] * 0.58))
    upper = gray[:upper_h, :]
    ...
    # One confidently detected eye is enough to classify the eyes as OPEN.
    return len(eyes) >= 1
    ...
    try:
        while True:
            ...
            if (
                ...
                if eye_result is not None:
                    # During startup we display the detector result but do not
                    # allow CLOSED samples to build a fatigue condition.
                    if now - startup_time < STARTUP_EYE_WARMUP:
                        if eye_result:
                            eye_history.append(True)
            ...
            if (
                ...
                try:
                    ...
                    if (
                        candidate_count
                        >= required_confirm_count(raw_state)
                    ):
                        # Fast response: one valid inference is enough.
                        # A short hold prevents rapid oscillation between states.
                        if (
                            raw_state == current_state
                            or now - state_since >= STATE_MIN_HOLD_SECONDS
                            or current_state == ""
                        ):
                            ...
                            shown_emotion = candidate_emotion
            ...
            if last_face is None:
                ...
                if (
                    nucleo is not None
                    and not off_sent_for_no_face
                    and now - no_face_since >= NO_FACE_OFF_SECONDS
                ):
                    if send_state(nucleo, "OFF"):
                        last_sent_state = "OFF"
                        last_send_time = now
                        off_sent_for_no_face = True
        ...
        try:
            if nucleo is not None:
                # Safety: always turn all outputs off when the application exits.
                try:
                    send_state(nucleo, "OFF")
                    time.sleep(0.15)
                except Exception:
                    pass
                nucleo.close()
